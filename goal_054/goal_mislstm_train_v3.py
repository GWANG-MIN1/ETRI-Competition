...
# --- HONEST nested check: choose per-target w on INNER split, apply to outer (no peeking) ---
print("\n=== HONEST nested blend (w chosen on first half, applied to second half) ===")
half = N // 2
oof_pred = pred  # already out-of-fold per-row
tot_o2 = tot_b2 = 0
for j, t in enumerate(TARGETS):
    y = YY[:, j]
    # tune w on rows [0:half], eval on [half:]
    bestw, bestl = 0.0, 1e9
    for w in np.linspace(0, 0.6, 13):
        bl = 1/(1+np.exp(-((1-w)*logit(OOF[:half, j]) + w*logit(oof_pred[:half, j]))))
        l = bll(y[:half], bl)
        if l < bestl: bestl, bestw = l, w
    bl2 = 1/(1+np.exp(-((1-bestw)*logit(OOF[half:, j]) + bestw*logit(oof_pred[half:, j]))))
    lo2 = bll(y[half:], OOF[half:, j]); lb2 = bll(y[half:], bl2)
    tot_o2 += lo2; tot_b2 += lb2
    print(f"  {t}: w*(innerhalf)={bestw:.2f}  OOF {lo2:.4f} -> blend {lb2:.4f}  ({lb2-lo2:+.4f})")
print(f"  HONEST MEAN: OOF {tot_o2/7:.4f} -> blend {tot_b2/7:.4f}  ({tot_b2/7-tot_o2/7:+.4f})")
